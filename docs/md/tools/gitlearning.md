...
Git 官方地址 [https://git-scm.com/downloads](https://git-scm.com/downloads "Git下载")，或者 [gitforwindows](https://gitforwindows.org/)，普通的下载方式比较慢，可以考虑用其他工具下载。

### 4.2 开始安装

打开下载好的安装包，开始安装，这里以2.39.0版本为例子。

![Git 启动安装](./images/git安装.PNG)

### 4.3 选择安装目录

点击“next”进入下一步，这里可以选择“Browse...”来自定义安装目录，这里选择默认路径。

![选择安装目录](./images/git默认安装路径.PNG)

### 4.4 选择安装组件

点击“next”进入下一步，进入组件选择界面，这里默认即可。

![组件选择](./images/git组件选择.PNG)

### 4.5 选择默认编辑器

点击“next”，会进入开始菜单选择，默认即可，会在开始菜单中生成Git的文件夹。继续下一步，进入编辑器选择界面，这里选择 VS code。

![编辑器选择](./images/git编辑器选择.PNG)

### 4.6 主分支名字选择

初始化新项目时，主分支的名字，选择让 Git 自己决定。

![主分支名字选择](./images/git主分支名字.PNG)

### 4.7 环境变量

选择默认，支持git bash和第三方命令提示符或power shell。

![环境变量](./images/git环境变量.PNG)

### 4.8 选择SSH

选择默认。

![SSH选择](./images/gitSSH选择.PNG)

### 4.9 选择HTTPS后端传输

作为普通用户，选择默认就可以了。

![HTTPS后端选择](./images/gitHTTPS后端选择.PNG)

### 4.10 配置行尾符号转换

windows下，选择windows-style。

![行尾符号配置](./images/git行尾换行符配置.PNG)

### 4.11 后续

后续的终端模拟器选择MinTTY，默认的git pull行为选择默认。在可能的情况下，将当前分支定位到获取的分支，否则创建合并提交。

安装完成后，打开cmd，输入`git --version`查看是否安装成功。

![安装成功](./images/git安装完成.PNG)

在VS code的终端中，输入同样的命令，查看是否可以在VS code中使用Git。

![VS code使用Git](./images/VScode查看git版本.PNG)
...
`.gitignore`是一个纯文本文件，包含了项目中所有指定的文件和文件夹列表，这些文件和文件夹是Git应该忽略的。如果不希望Git跟踪某些目录或文件，需要编辑`.gitignore`文件。

`.gitignore`的一些基本模式的示例：

1. 忽略根目录名为`test.txt`的文件。

    `/test.txt`

2. 忽略一个位于根目录下的`test`目录下的`test.txt`文件。

    `/test/test.txt`或者`test/test.txt`

3. 忽略所有具有特定名称的文件，这种模式会忽略项目中任何位置具有这个名称的文件。
